# World Cup 2026 — Hidden Transfer Gems
...
**Output:** Top 5 transfer opportunities per position (FW, MF, DF, GK), forming a recommended shortlist for further scouting.
...
Extracted player statistics from FBref by saving all 48 team pages as HTML files. Python (`pandas.read_html`) parsed 5 statistical tables per team:
...
Transfermarkt data was collected separately: market value, age, height, foot, position.

A Kaggle dataset was evaluated and **rejected** — it contained fabricated player names and players who were not in the actual squads.

### Step 2 — Data Cleaning (Python)

- Fixed multi-level headers from FBref HTML tables
- Standardised country names across sources (e.g. `"Korea Repulic"` → `"Korea Republic"`, `"Congo Dr"` → `"DR Congo"`)
- Removed duplicate rows, summary rows (`"Squad Total"`, `"Opponent Total"`), and blank entries
- Converted text columns to numeric types
- Assigned a primary position for dual-position players (`"FW,MF"` → `"FW"`)

### Step 3 — Data Merging

- Created normalised name keys: lowercase, accents removed, hyphens stripped
- Merged FBref + Transfermarkt on `match_name` + `Country`
- Resolved 213 name mismatches through automated normalisation and 7 rounds of manual correction
- Korean player names required reversal (family name first vs given name first)
- **Final match rate: 99.8%** — only 2 players unmatched, both with no Transfermarkt profile
...
| Age (inverse — younger is better) | 25% | Younger players offer more upside and resale value |
...
**This is not a list of the tournament's best players.**

It is a data-informed scouting shortlist identifying players who:
...
- **Team effects.** Individual statistics are influenced by team tactics, formation, and opposition quality.
...
- **Weights require justification.** The equal 10% weighting across performance metrics is a simplification. A production model would optimise weights against historical transfer outcomes.

---

## Future Improvements

- Validate the model against actual post-tournament transfers — did the shortlisted players move, and for how much?
- Replace equal metric weights with weights derived from historical transfer success
- Extend beyond the tournament to full-season club data for a larger sample per player
...
├── data/
│   ├── transfermarkt_all.csv
│   ├── merged_dataset.csv
│   ├── ranked_forwards.csv
│   ├── ranked_midfielders.csv
│   ├── ranked_defenders.csv
│   └── ranked_goalkeepers.csv
├── scripts/
│   ├── extract_data.py
│   ├── clean_and_merge.py
│   └── scoring_model.py
├── dashboard/
│   └── WC2026_Hidden_Gems.pbix
...
2. Place FBref HTML pages in the `perfwc26/` folder and Transfermarkt pages in `tmwc26/`
...
4. Open `dashboard/WC2026_Hidden_Gems.pbix` in Power BI Desktop
